...
"""Plot benchmark curves of CLC algorithm."""
...
def calc_clc_eff_fps(convert_info, context_frames, current_frame, msg_VIR):
    """Count times algorithm can run in specific interval."""
    begin_time = time.monotonic_ns()
    count_process_times = 0
    sexp = cooperative_lane_change.CooperativeLaneChange()
    while (time.monotonic_ns() - begin_time) / 1000000000 <= process_interval:
        ...
        _ = sexp.run(
            convert_info, context_frames_new, current_frame_new, msg_VIR
        )
        count_process_times += 1
    return count_process_times


def calc_clc_eff_time(convert_info, context_frames, current_frame, msg_VIR):
    """Record the time spent processing a frame of data."""
    begin_time = time.monotonic_ns()
    sexp = cooperative_lane_change.CooperativeLaneChange()
    context_frames_new = copy.deepcopy(context_frames)
    current_frame_new = copy.deepcopy(current_frame)
    _ = sexp.run(convert_info, context_frames_new, current_frame_new, msg_VIR)
    process_time = time.monotonic_ns() - begin_time
    return process_time / 1000000


def draw_clc_fps(x, y):
    ...
    plt.title("CLC Algorithm Benchmark", fontsize=15)


def draw_clc_time(x, y):
...
def draw_benchmark_fps(convert_info, context_frames1, current_frame1, msg_VIR):
    ...
    draw_clc_fps(benchmark_point_x, benchmark_point_y)
    plt.savefig("clc_algo_benchmark_fps.png", dpi=300)
    plt.savefig("clc_algo_benchmark_fps.svg")
    plt.show()
...
def draw_benchmark_time(
    ...
    plt.figure(figsize=(8, 5))
    draw_clc_time(benchmark_point_x, benchmark_point_y)
    plt.savefig("clc_algo_benchmark_time.png", dpi=300)
    plt.savefig("clc_algo_benchmark_time.svg")
    plt.show()
