...
class CongestionWarning:
    ...
    def __init__(self, kv, mqtt, mqtt_conn=None, node_id=None):
        ...
        self.ranges = [self.max_con_range,
                       self.mid_con_range,
                       self.min_con_range]

        self.rectangles = [[[rect[0], rect[0]], [rect[1], rect[1]]] for rect in self.ranges]
    ...
    def is_overlap(self):
        ...
        for i in range(len(self.rectangles)):
            for j in range(i+1, len(self.rectangles)):
                ...
                # 检查矩形的边界是否有重叠
                if rect1[0][0] <= rect2[1][0] and rect1[1][0] >= rect2[0][0] and rect1[0][1] <= rect2[1][1] and rect1[1][1] >= rect2[0][1]:
                    return True
    ...
    async def run(self, rsu: str, latest_frame: dict, _: dict = {}) -> dict:
        ...
        cgw, show_info, last_ts, CG_KEY = self._exe.run(
            ...
            self.min_con_range, self.mid_con_range, self.max_con_range
        )
