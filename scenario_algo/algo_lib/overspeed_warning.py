...
class OverspeedWarning(Base):
    ...
    def run(
        ...
        current_df["speed"] = current_df["speed"].apply(lambda x: int(x/0.02))
        # 获取最高限速
        ...
        current_df = current_df.dropna(
            axis=0, subset=["vehicleMaxSpeed"])  # 删除指定列中有缺失值的那一行数据
        df = current_df[np.where(current_df["speed"] >  # type: ignore
                                 current_df['vehicleMaxSpeed'], True, False)]  # type: ignore
    ...
    def _get_max_speed_limit(self, lane):
        if self.intersection_id in post_process.max_speed_limit:
            return post_process.max_speed_limit[self.intersection_id][lane]
        db.get_map_info()
        return post_process.max_speed_limit[self.intersection_id][lane]
    ...
    def _build_osw_event(self, df_res):
        ...
        self._overspeed_warning_message.append(
            info_for_osw
        )
