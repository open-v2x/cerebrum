...
from scenario_algo.slowspeed_external import slowspeed_grpc_pb2
from scenario_algo.slowspeed_external import slowspeed_grpc_pb2_grpc
from websockets.client import connect


class Base:
    """Super class of slowspeed warning class."""
...
class SlowspeedWarning(Base):
    ...
    def __init__(self) -> None:  # noqa
        self.endpoint_config = (
            modules.algorithms.slowspeed_warning.endpoint_config
        )
        self.connect = None
    ...
    async def run(
        ...
        try:
            msg, show_info, last_timestamp_res = await getattr(
    ...
    async def _run_from_external_grpc(self, data):
        stub = slowspeed_grpc_pb2_grpc.SlowSpeedGrpcStub(self.connect)
        response = await stub.slow_speed(
            slowspeed_grpc_pb2.SlowSpeedRequest(data=json.dumps(data))
        )
