...
class CheckLoggingFormatArgs(BaseASTChecker):
    ...
    def visit_Call(self, node):
        ...
        if isinstance(node.func, ast.Attribute):
            ...
            if obj_name != "LOG" or method_name not in self.LOG_METHODS:
                return super(CheckLoggingFormatArgs, self).generic_visit(node)
...
def check_explicit_underscore_import(logical_line, filename):
    ...
    if filename in UNDERSCORE_IMPORT_FILES:
        ...
        underscore_import_check.match(logical_line)
        or underscore_import_check_multi.match(logical_line)
...
class CheckForStrUnicodeExc(BaseASTChecker):
    ...
    CHECK_DESC = (
        "C314 str() and unicode() cannot be used on an "
        "exception.  Remove it."
    )
...
class CheckForTransAdd(BaseASTChecker):
    ...
    CHECK_DESC = (
        "C315 Translated messages cannot be concatenated.  "
...
def validate_assertTrue(logical_line):
    ...
    if re.match(assert_True, logical_line):
        msg = (
            "C312: Unit tests should use assertTrue(value) instead"
...
def validate_assertIsNone(logical_line):
    ...
    if re.match(assert_None, logical_line):
        msg = (
            "C311: Unit tests should use assertIsNone(value) instead"
